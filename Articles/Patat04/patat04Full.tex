...
général ces logiciels vont avoir comme entrée un ou plusieurs
fichiers, leur sortie est constituée d'un ou plusieurs fichiers.
Travailler de cette manière pose au moins deux problèmes~: le
premier est qu'il faut construire les fichiers en entrée à chaque
fois qu'un horaire est produit; le deuxième est la prise en compte
des changements. Le premier problème peut avoir deux cas~:
production manuelle des fichiers ou production automatisée. si les
fichiers sont produits manuellement les erreurs de frappe ou
d'inattention peuvent produire des effets inattendus dans le
logiciel et sont difficiles à trouver, car il faut vérifier le
tout manuellement. Dans le cas, où les fichiers sont produits de
manière automatique, il faut que le logiciel s'assurer de vérifier
la validité et la cohérence des toutes les données. Le deuxième
problème se manifeste lorsqu'un changement est introduit dans le
...
est utilisé à plusieurs sites différents sans une compatibilité
...
modifiables (extensibles) le coût de la solution sera faible, mais
si le système est monolithique et fermé une modification sera très
coûteuse.
...
mentionnées en réduisant le coût de modifications et en validant
les données à l'endroit nécessaire dans le processus.
L'architecture met en \oe{}uvre des bases de données, des sites
Web et un logiciel de construction d'horaires. La base de données
va aider à vérifier la validité des données au moment de la
saisie, elle va fournir les données nécessaires au logiciel de
construction d'horaires dans le format nécessaire. Les sites Web
sont utilisés principalement pour la saisie de données, ceci
permet de vérifier aussi la validité des données avant que la
donnée soit entrée dans la base. Autre fonction du site Web est
d'offrir les horaires de manière personnalisé. Avec l'aide du
logiciel de construction d'horaires, il est possible de fabriquer
un horaire avec un minimum de conflits.
...
et al \cite{decausmaecker02} ont introduit l'idée qu la sémantique
du Web peut être utilisé par le chercheurs en construction
d'horaires. Ils ont introduit l'idée d'utiliser XML comme langage
de communication. Nous proposons aussi l'utilisation de XML afin
de faciliter les échanges entre certains modules de notre
architecture. Dans Burke et al. \cite{burke97} il est proposé
d'avoir un standard pour le format de données pour des instances
de horaires, leur but était de comparer des algorithmes
...
semble qu'il serait souhaitable d'avoir un standard, mais le but
très difficile à atteindre. Notre approche permet soit au niveau
de la base de données de générer un fichier ou données respectant
un format défini, soit de générer différents fichiers pour
...
Une fois l'architecture définie, nous l'avons crée une instance
afin de l'implanter dans système de production d'horaires à
...
que à l'Université de Sherbrooke il existe des Facultés où les
étudiants font un pré-choix et d'autres où les horaires sont fait
sans pré-choix. Le pre-choix des étudiants fait partie des données
à saisir, s'il n'y a pas de pré-choix, il faut indiquer le nombre
d'étudiants à admettre pour chaque cours.
...
La figure \ref{fig:fOne} est une vue de très haut niveau de la
production d'horaires, le système de production inclue le logiciel
de construction d'horaires. Dans le système que nous proposons,
nous essayons de limiter le plus possible d'intervention humaine
afin de réduire des erreurs de manipulation. Nous essayons de
réduire aussi le sources possible d'erreurs en vérifiant l'entré
de données.

\begin{description}
    \item[Système de production d'horaires] Il est composé de plusieurs logiciels,
ou modules, qui fonctionnent de manière autonome, mais coordonnée.


    \item[\pro{Données}] représentent des données nécessaires à la production
d'horaires. Nous allons sauvegarder tous les données nécessaires
dans un base de données. Dans le schéma il n'est pas indiqué que
ces données peuvent être manipulés par des modules du système de
production d'horaires. Par exemple, le module de saisie de
pre-choix, va sauvegarder les cours suivis par un étudiant. Le
...
obtenues seront remises dans la base.
 ...
 \item[\pro{Papier} ou \pro{Web}] Il s'agit de la sortie du système.
Ces sorties peuvent être des fichiers produits sur papier ou des
données affichés sur une page Web, dans le deux cas les données
sources viendront de la base de données.

\end{description}

Il nous apparaît clair que  le système doit avoir comme composante
principale  une base de donnée.
...
\begin{description}
    ...
    caractéristiques de salles de cours, étudiants.
...
informations nom et disponibilité dans la base.
...
de manière manuelle, ave construit avec des hypothèse
...
\end{picture}
\caption{L'architecture trois couches.}\label{fig:phThree}
\end{figure}
...
dans la figure \ref{fig:phFour}.

\begin{figure}
\setlength{\unitlength}{1cm}
\begin{picture}(4.5,8)
  \put(1.5,7.7){\makebox(0,0){Interface utilisateur}}
  \put(0,6.5){\framebox(3.5,1){Saisie de données}}
  \put(3.5,7.1){\vector(1,-1){1.5}}
  \put(5,5.6){\vector(-1,1){1.5}}

  \put(1.5,4.2){\makebox(0,0){Interface utilisateur}}
  \put(0,3){\framebox(3.5,1){Autres systèmes}}
  \put(3.5,3.6){\vector(1,1){1.5}}
  \put(5,5.1){\vector(-1,-1){1.5}}

  \put(1.5,6){\makebox(0,0){Interface utilisateur}}
  \put(0,4.8){\framebox(3.5,1){Production d'horaires}}
  \put(3.5,5.2){\vector(1,0){1.5}}
  \put(5,5.2){\vector(-1,0){1.5}}

  \put(6.2,6){\makebox(0,0){Logique d'affaires}}
  \put(5,4.8){\framebox(2.5,1){Serveur Web}}
  \put(7.5,5.2){\vector(1,0){1}}
  \put(8.5,5.2){\vector(-1,0){1}}

  \put(10.2,6){\makebox(0,0){Persistance des données}}
  \put(8.5,4.8){\framebox(3.7,1){Serveur base de données}}


\end{picture}
\caption{// L'implementation de l'architecture trois couches pour
le système d'horaires.}\label{fig:phFour}
\end{figure}
...
d'une manière presque arbitraire. Pour sa construction nous avons
pris en considération une certaine logique, nous avons mit
ensemble les éléments qui ont des caractéristiques communes ou
bien qui appartiennent à un même fabricant de logiciel.
...
la logique d'affaires. Dans le tableau \ref{tb:tbOne}, nous
...
virtuelle pour plusieurs plateformes différentes, ce qui signifie
que les programmeurs peuvent écrire pour ce langage en utilisant
une plateforme, et faire tourner leurs programmes sur autres.
...
nous utilisons Java comme langage pour établir la conexion vers la
base de données, nous pouvons utiliser JDBD (Java Database
...
modèlisation de la production d'un horaire, le processus actuel de
