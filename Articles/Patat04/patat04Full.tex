...
d'horaires; dans d'autres commence par l'automatisation des
traitements associés au fonctionnement de l'institution, comme par
exemple le suivit des étudiants. Il existe de logiciels pour
effectuer la construction d'horaires, on peut citer comme exemples
\cite{gonzalezrubio00,ferland94}, mais, il existe d'autres.

En général, les logiciels pour la création des horaires utilisent
...
fichiers est une sources de problèmes. Il faut construire les
fichiers en entrée à chaque fois qu'un horaire est produit. Il
...
difficiles à trouver, car il faut vérifier toutes les données de
fichiers manuellement. Dans le cas, où les fichiers sont produits
de manière automatique, il faut que le logiciel s'assurer de
vérifier la validité et la cohérence des toutes les données. Le
deuxième problème se manifeste lorsqu'un changement est introduit
dans le système (soit dans l'application de construction, soit
dans celui de la production de fichiers) ou si l'application de
construction est utilisé à plusieurs sites différents, utilisant
de formats de données différents.

Une autre source de problèmes encore plus coûteuse que la
précédente, est celle de l'introduction d'un changement dans le
système, qui entraîne une modification du logiciel de construction
ou du logiciel de la production ou dans les deux. Les changements
...
d'horaire. Il s'agit d'un problème général dans le développement
de logiciels.


La solution pour le premier problème passe par l'automatisation de
la vérification et validation de données à la saisie des données,
...
modification sera très coûteuse. Sa solution est le développement
de logiciel ayant, entre autres, la qualité d'extensibilité.
...
aider à faire face plus facilement aux deux problèmes mentionnées
en réduisant le coût de modifications et en effectuant la
vérification et la validation des données à l'endroit nécessaire
dans le processus et en évitant la redondance du code et des
données. Cette architecture met en \oe{}uvre une bases de données,
un sites Web et un logiciel de construction d'horaires.  La
vérification et la validation de données  se fera à l'entrée de
données et avec de contraintes d'intégrité dans la base de
données.  Les sites Web sont utilisés principalement pour la
...
personnalisé, une fois que l'horaire est construit. La base de
données permet l'extraction des données, selon différents formats.
Ainsi, l'entrée de données du logiciel de construction est
facilité.  Bien entendu, avec l'aide du logiciel de construction
d'horaires, il est possible de fabriquer un horaire avec un
minimum de conflits.
...
jusqu'à présent, au développement général de la production
...
utilisé come un moyen de diffusion de horaires. Nous allons un peu
...
ont introduit l'idée que la sémantique du Web peut être utilisé
par le chercheurs en construction d'horaires. Ils ont aussi
introduit l'idée d'utiliser XML comme langage de communication
dans le construction d'horaires par agents. Nous proposons aussi
...
de données pour des instances de horaires, leur but était de
comparer des algorithmes (programmes) dans de benchmarks. Dans un
travail plus récent, Kingston \cite{kingston02} propose un
...
À la place, notre approche permet d'utiliser  la base de données
...
l'architecture, blocs de base et la responsabilité de chaque bloc
en évitant les traitements redondants. Les objectifs de définir
une architecture ouverte et extensible ont été respectes pendant
...
l'Université de Sherbrooke. Nous avons fait un survol de
technologies afin de faire les bons choix respectant encore les
...
quatre mois), donc le système doit s'adapter à tout periode $P$.}.
Le processus fini lorsque toutes le personnes concernés reçoivent
l'horaire sur un support papier ou sur le Web, voir figure
\ref{fig:fOne} et que la période de validité de l'horaire touche à
sa fin. Les données à saisir sont multiples, par exemple les cours
qui vont avoir lieu dans la période $P$; le format du cours (3h,
ou 2h+1h); les enseignants possibles ou assignées à certains
cours; les locaux possibles ou assignées; la disponibilité des
enseignants et des locaux, etc.. Signalons qu'il existe des
institutions où les étudiants font un pré-choix et d'autres où les
horaires sont fait sans pré-choix. Le pre-choix des étudiants fait
partie des données à saisir, s'il n'y a pas de pré-choix, il faut
indiquer le nombre d'étudiants à admettre pour chaque cours.
...
d'horaires, le système de production inclue un logiciel de
construction d'horaires.
...
doit avoir deux composantes principales~: une base de données et
le logiciel de construction d'horaires.

\item[Données] représentent des données nécessaires à la
production d'horaires. Nous allons sauvegarder toutes les données
nécessaires dans une base de données. Dans la figure, il n'est pas
...
\section{L'architecture en couches} \label{sec:archi}
...
l'application de production d'horaires, dans cette section nous
allons présenter l'integration entre le flot des données propre à
notre système de production d'horaires et l'architecture proposé.
...
la section antérieure, nous allons proposer un composant qui fait
partie de l'application de production d'horaires. Dans cette
section nous allons montrer l'intégration entre le flot des
...
\item Les données générées par autres systèmes (données de base).
Depuis long temps il est prouvé que la meilleure manière
...
sortent de notre système, et ils sont envoyés vers autres systèmes
ou bien vers autres utilisateurs, par exemple, les étudiants qui
vont recevoir un papier avec son horaire personnalisé ou bien, il
peut consulter cette information en utilisant un site Web crée
pour ces fins, ou bien, l'organisme d'administration central de
l'institution qui doit être informé de l'affectation des horaires.

\end{enumerate}

Il faut tenir en considération que la couche d'interface
d'utilisateur communique uniquement avec la couche de logique
d'affaires. Une application n'est pas capable d'ajouter, modifier
ou effacer des données directement dans la base de données, elle
doit passer irrémédiablement par la couche de logique d'affaires.
Cette caractéristique nous permet d'assurer la cohérence des
données.
