...
système, soit dans le logiciel de construction, ou dans logiciel
de la production ou bien dans les deux. Les changements viennent
de la part de responsables de l'institution, par exemple une
nouvelle nomenclature pour les locaux; nouvelle approche
...
modification sera très coûteuse. Une manière de diminuer les
effets de ce problème est le développement de logiciel ayant,
...
mentionnées, en réduisant le coût des modifications et en
...
d'horaires. La vérification et la validation de données  se fera à
l'entrée de données et avec de contraintes d'intégrité dans la
base de données.  Le site Web est utilisé principalement pour la
saisie de données, ceci permet de vérifier aussi la validité des
données, grace au règles d'affaires avant que la donnée soit
entrée dans la base. Autre fonction du site Web est d'offrir les
horaires de manière personnalisé, une fois que l'horaire est
construit. La base de données permet l'extraction des données,
selon différents formats. Ainsi, l'entrée de données du logiciel
de construction est facilité. Bien entendu, avec l'aide du
...
consacrés, jusqu'à présent, au développement général de la
production d'horaires. Nous pensons que le fait d'avoir une vue
globale sur le processus peut aider la recherche en construction
d'horaires. White \cite{white00} a mis en évidence que le Web doit
être utilisé comme un moyen de diffusion d'horaires. Nous allons
un peu plus loin en proposant  de l'utiliser également comme
...
d'utiliser une base de données comme un outil d'extraction et de
...
encore les objectifs d'ouverture et extensibilité ont été
atteints. Ceci est présenté dans les sections \ref{sec:tech} et
\ref{sec:tictac}.
...
\section{Les flot de l'information dans la production d'horaires}
\label{sec:data}
...
Le processus fini lorsque toutes les personnes concernées
reçoivent l'horaire sur un support papier ou sur le Web (voir
figure \ref{fig:fOne}) et que la période de validité de l'horaire
touche à sa fin. Les données à saisir sont multiples, par exemple
les cours qui vont avoir lieu dans la période $P$; le format du
cours (3h, ou 2h+1h); les enseignants possibles ou assignées à
certains cours; les locaux possibles ou assignées; la
disponibilité des enseignants et des locaux, etc.. Signalons qu'il
existe des institutions où les étudiants font un pré-choix et
d'autres où les horaires sont fait sans pré-choix. Le pre-choix
...
doit avoir deux composantes principales~: le système de sauvegarde
de données et le logiciel de construction d'horaires.
...
\begin{description}
    ...
    ce type d'information. Il s'agit en général des informations qui sont partagées par
...
construction de l'horaire et qui interagit avec  le programme de
construction d'horaires.
...
\item [La couche de persistance~:] Gère le stockage physique des
données, soit dans des fichiers avec un certain format, ou dans un
système de bases de données traditionnelle ou bien dans d'autres
...
\begin{picture}(19,6)
  ...
  \put (0,4){\line(1,0){22}}
  \put (19,4.5){\makebox(0,0){\scriptsize Interface utilisateur}}
  \put(0,4.5){\framebox(4,1){\tiny Saisie de données}}
  \put(2,4.65){\makebox(0,0){\tiny (Client web)}}
  \put(5,4.5){\framebox(4,1){\tiny Création d'horaires}}
  \put(10,4.5){\framebox(4,1){\tiny Autres systèmes}}


  \put (0,2){\line(1,0){22}}
  \put (19,2.5){\makebox(0,0){\scriptsize Logique d'affaires}}
  \put (2,2.5){\framebox(4,1){\tiny Serveur Web}}
  \put (8,2.5){\framebox(4,1){\tiny Règles d'affaires}}

  \put (0,0){\line(1,0){22}}
  \put (19,0.5){\makebox(0,0){\scriptsize Persistance des données}}
