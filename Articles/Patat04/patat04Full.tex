...
beaucoup d'institutions d'enseignement. Ces institutions
...
d'horaires, on peut citer comme exemple \cite{gonzalezrubio00}. En
générale ces logiciels vont avoir comme entrée un ou plusieurs
fichiers, la sortie est aussi composée d'un ou plusieurs fichiers.
Travailler de cette manière pose au moins deux problèmes~: le
premier est qu'il faut construire les fichiers à chaque fois qu'un
horaire sera produit; le deuxième est que les changements pour
prendre en compte une nouvelle information sont difficiles, il
faut changer le format de données et l'application. Le premier
problème est plus grand lorsque les fichiers sont produits
manuellement; si les fichiers sont produits de manière
automatique, il faut vérifier la validité des données. Le deuxième
problème ne se manifeste que s'un changement doit être introduit
dans le système (logiciel et production de fichiers)ou si le
logiciel est utilisé à plusieurs sites différents. Les changements
viennent de la part des utilisateurs, par exemple une nouvelle
nomenclature pour les locaux; prise en compte de nouvelles données
pour former des groupes (toutes les filles d'une promotion doivent
être dans le même groupe), etc.. S'il existe plusieurs instances
du logiciel, il faut qu'il puisse traiter les données des
différents sites ou que les sites adoptent le format de fichiers.

Dans cet article nous proposons l'architecture  d'un système de
production des horaires de cours et des examens ayant les
caractéristiques suivantes~: ouverte et extensible. L'architecture
met en oeuvre des bases de données, des sites Web et le logiciel
de construction d'horaires. La bases de données va servir à
vérifier la validité des données, elle va fournir les données
nécessaires au logiciel de construction d'horaires et s'occupe du
formatage de données. Les sites Web sont utilisés principalement
pour la saisie de données, ceci permet de vérifier aussi la
validité des données.

Nous avons procédé en deux étapes, nous avons analysé le flot des
données dans un système générique de production d'horaires, ceci
nous a permis d'identifier les interfaces du système afin d'avoir
une architecture ouverte. Ensuite nous avons défini
l'architecture, ses modules et la responsabilité de chaque module.

Une fois l'architecture définie nous proposons une instance en
utilisant des technologies existantes.
