...
Une fois l'architecture définie, nous l'avons créer une instance,
adapté au système implanté à l'Université de Sherbrooke, en
...
Beaucoup de recherche est faite dans la construction d'horaires
algorithmes et formulations mathématiques et plusieurs logiciels
ont été proposés voir \cite{patat97,patat00,patat02}, peu
d'efforts ont été consacrés au problème de la production
d'horaires, 
 . Burke et al. \cite{burke97} il est proposé d'avoir
un Standard pour le format de données pour des instances de
horaires, leur but était de comparer des algorithmes (programmes)
dans de benchmarks. \cite{ozcan03} suggère aussi un autre standard
cette fois avec le langage XML. Il nous semble que bien que
souhaitable un standard est un but très difficile à atteindre.
Notre approche permet soit au niveau de la base de données de
générer un fichier ou données respectant un format défini, soit de
générer différents fichiers pour différents programmes.
