...
manuellement. Dans le cas, où les fichiers sont produits de
manière automatique, il faut que le logiciel s'assure de vérifier
...
est utilisé à plusieurs sites différents, utilisant de formats de
données différents.
...
un site Web et un logiciel de construction d'horaires.  La
vérification et la validation de données  se fera à l'entrée de
données et avec de contraintes d'intégrité dans la base de
données.  Le site Web est utilisé principalement pour la saisie de
données, ceci permet de vérifier aussi la validité des données,
grace au règles d'affaires avant que la donnée soit entrée dans la
base. Autre fonction du site Web est d'offrir les horaires de
manière personnalisé, une fois que l'horaire est construit. La
...
est facilité.  Bien entendu, avec l'aide du logiciel de
...
utilisé comme un moyen de diffusion de horaires. Nous allons un
peu plus loin en proposant  de l'utiliser également comme
interface d'entrée, de plus nous proposons l'architecture
nécessaire à supporter nos idées. De Causmaecker et al
\cite{decausmaecker02} ont introduit l'idée que la sémantique du
Web peut être utilisé par le chercheurs en construction
d'horaires. Ils ont aussi introduit l'idée d'utiliser XML comme
langage de communication dans le construction d'horaires par
agents. Nous proposons aussi l'utilisation de XML, afin de
faciliter les échanges entre certains modules de notre
...
but très difficile à atteindre. À la place, notre approche permet
d'utiliser  la base de données comme un outil d'extraction et de
formattage, de cette manière le logiciel de construction
d'horaires peut recevoir les données nécessaires. Ainsi, il serait
possible de générer des instances de problèmes pour d'autres
logiciels de construction d'horaires.
...
l'architecture mieux adapté à ce flot d'information. Les objectifs
de définir une architecture ouverte et extensible ont été
respectes pendant nos travaux. Ceci est présenté dans les sections
...
(3h, ou 2h+1h); les enseignants possibles ou assignées à certains
...
doit avoir deux composantes principales~: système de sauvegarde de
données et le logiciel de construction d'horaires.
...
mieux résoudre notre problème est celle en couches.
...
le Tableau \ref{tb:tbOne} n'est pas exhaustive.
...
d'interface utilisateur pour pouvoir interagir avec des fichiers
en format XML. Après, si nous voulons faire évoluer cette partie
de persistance de données vers un système de bases de données, il
