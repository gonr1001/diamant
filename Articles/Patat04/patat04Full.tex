...
beaucoup d'insti\-tutions d'enseignement. Le coût de cette tâche
peut être important dans certaines institutions. Ces institutions
...
le format de fichiers. Le coût de ces problèmes est le coût de la
modification dans le logiciel. Il peut être élevé si les logiciels
ne sont pas préparés, c'est-à-dire si le logiciels n'ont pas été
conçus pour être modifiables (extensibles). L'autre facteur qui
peut augmenter le coût est de modifier un système monolithique ou
fermé, une modification peut affecter tout le système.
...
L'architecture met en oeuvre des bases de données, des sites Web
et le logiciel de construction d'horaires. La base de données va
aider à vérifier la validité des données au moment de la saisie,
elle va fournir les données nécessaires au logiciel de
...
interpreteur, avec le même objectif. Özcan \cite{ozcan03} suggère
...
présenté dans la sections \ref{sec:tictac}.
...
voir figure \ref{fig:fOne}. Les données à saisir sont les cours
qui vont avoir lieu dans la période $p$. Le format du cours (3h,
ou 2+1h). Les enseignants possibles ou assignées à certains cours,
les locaux possibles ou assignées. La disponibilité des ensignants
et des locaux. Signalons que à l'Université de Sherbrooke il
existe des Facultés où les étudiants font un pré-choix et d'autres
où les horaires sont fait sans pré-choix. Le pre-choix des
étudiants fait partie des données à saisir, s'il n'y a pas de
