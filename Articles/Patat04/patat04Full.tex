...
\begin{picture}(5,8)
  ...
  \put(0,3){\framebox(3.5,1){Production d'horaires}}
  \put(3.5,3.6){\vector(1,1){1.5}}
  \put(5,5.1){\vector(-1,-1){1.5}}

  \put(6.5,6){\makebox(0,0){Logique d'affaire}}
  \put(5,4.8){\framebox(3.5,1){Serveur Web}}
  \put(8.5,5.2){\vector(1,0){1.5}}
  \put(10,5.2){\vector(-1,0){1.5}}

  \put(11.5,6){\makebox(0,0){Logique d'affaire}}
  \put(10,4.8){\framebox(3.5,1){Serveur Web}}


\end{picture}
\caption{L'architecture trois tiers.}\label{fig:phFour}
\end{figure}
...
appartiennent à un même fabricant de logiciel.
...
effectuer quelques changements dans l'élément d'interface
utilisateur pour pouvoir interagir avec des fichiers en format
XML. Après, si nous voulons faire évoluer cette partie de
persistance de données vers un système de bases de données, il
faudrait faire des changements dans la couche d'interface
d'utilisateur et programmer l'élément de persistance des données.
Tous ces changements sans jamais toucher l'élément de la logique
d'affaires.

Dans le tableau suivante (voir Tableau:\ref{tb:tbOne}) nous
