...
Le commis est celui qui va saisir les infrastructures comme dans
la base, le responsable et celui qui est responsable de la
construction l'horaire et qui interagit avec  le programme de
construction d'horaires.
...
Par exemple si la donne attendue est un entier dans une
intervalle, et que la donnée entrée correspond à l'ensemble de
valeurs attendues; elle sera entrée dans la base. Sinon. le commis
doit rentrer une nouvelle valeur. Bien entendu, il faut avoir
...
de respecter les contraintes fortes et de essayer de respecter le
...
nous sommes en mesure de proposer une architecture pour implénter
le système
...
composition et les contraints de ces modèles. \cite{shaw96}
...
de notre problème complexe \cite{buschmann96}.

À l'heure actuelle il y a plusieurs architectures disponibles pour
développer des applications logicielles,  la plus adaptée pour
mieux résoudre notre problème est celle de trois couches.
...
ou dans un système de bases de données traditionnelle ou bien dans
...
présenté, nous allons proposer un composant qui fait partie de
...
venir de la saisie directe des utilisateurs (données d'une
période) ou bien des données qui sont générées par autres systèmes
(données de base). Nous avons aussi des données qui sortent de
notre système comme résultat de la création d'horaires (données
horaire) (voir figure \ref{fig:fTwo})
...
institution, par exemple, la quantité maximale d'étudiants dans
