...
exemple le suivi des étudiants. Il existe plusieurs logiciels pour
effectuer la construction d'horaires, on peut citer comme exemple
DIAMANT \cite{gonzalezrubio00} et SAPHIR \cite{ferland94}.
...
automatique, il faut que le logiciel vérifié la validité et la
cohérence de toutes les données.

Une autre source de problèmes, encore plus coûteuse que la
précédente, est celle de l'introduction d'un changement dans le
système, soit dans le logiciel de construction, dans le logiciel
de la production ou bien dans les deux. Les changements viennent
de la part des responsables de l'institution, par exemple une
...
traiter. Si l'ensemble des logiciels de production a été conçu
pour être modifiable (extensible) le coût associé à une
modification sera faible. Mais si le système est monolithique et
fermé, une modification sera très coûteuse. Le développement de
logiciel ayant, entre autres, la qualité d'extensibilité, diminue
les effects de ce problème.
...
mentionnés précédemment  en réduisant le coût des modifications,
...
de données, un site Web et un logiciel de construction d'horaires.
La vérification et la validation de données  se font à l'entrée de
...
de données, ce qui permet de vérifier aussi la validité des
données grâce aux règles d'affaires, avant que la donnée soit
entrée dans la base. Une autre fonction du site Web est d'offrir
les horaires de manière personnalisée, une fois que l'horaire est
...
de construction est facilitée. Bien entendu, avec l'aide du
logiciel de construction d'horaires, il est possible de fabriquer
un horaire avec un minimum de conflits.


De nombreuses recherches ont été faites dans la construction
...
de données pour des instances d'horaires, leur but étant de
...
bloc qui participe à l'architecture. Les objectifs de définir une
architecture ouverte et extensible ont guidé nos travaux. Ceci est
présenté dans les sections \ref{sec:data} et \ref{sec:archi}.
...
Le choix de la base est une décision d'implémentation. De manière
...
logiciel ou le responsable ne trouve pas une solution
satisfaisante il faut faire des changements même dans certains
données fixées.

Ces activités de saisie et construction d'horaires sont effectues
par plusieurs utilisateurs. Chacun a un rôle et des privilèges
associés à ce rôle pour changer certaines données. Afin de
simplifier, nous considérons deux types d'utilisateurs~: le commis
et le responsable de la construction. Le commis saisit les
informations dans la base de données, le responsable de la
...
changements effectués pour chaque itération et faire un ``commit''
lorsque la solution est choisie et que l'horaire devient définitif
pour la période.
...
À la suite de l'étude du flot de données dans la production
d'horaires, nous sommes en mesure de proposer une architecture
...
les
développeurs\footnote{http:\//\//www.webopedia.com\//TERM\//O\//open\_architecture.html}.
...
langages sont fortement utilisés pour la création de pages Web, et
nous voulons profiter de tous les bénéfices qu'offre un site Web
pour une application de type transactionnelle.
...
La réalisation du système semble complexe. Mais, nous pouvons
reprendre plusieurs modules existants. C'est cela un des avantages
de l'ouverture et l'extensibilité de l'architecture et de
l'instance. Nous pouvons analyser par couche les composantes à
réutiliser et ceux à implémenter :

\begin{description}

\item [Couche Interface.] Pour cette couche, il y a un élément
technologique qui facilite le travail énormément, le navigateur
Web. Cet outil est disponible dans la plupart des ordinateurs qui
ont accès à l'Internet. Il faut alors, centraliser les efforts en
développer des pages Web avec la qualité de compatibilité entre
deux navigateurs Web: Internet Explorer\footnote{
http:\//\//www.microsoft.com} et Netscape\footnote{
http:\//\//www.netscape.com}.

Le site Internet http://validator.w3.org/, offre un outil
d'analyse de compatibilité avec le standard HTML pour les sites
Web, cet outil est fourni par le ``World Wide Web
Consortium''\footnote{ http:\//\//www.w3.org/}.
...
Le projet ''Apache HTTP'' est un effort de développer et maintenir
un serveur HTTP ouvert ainsi que ''Open source'' pour les systèmes
d'exploitation plus utilisés comme UNIX et Windows. Le but de ce
projet est de fournir un serveur sécurisé, efficace et extensible
capable d'offrir des services HTTP selon les normes HTTP
courantes.
...
Jakarta-Tomcat est un conteneur de servlets.

Nous pouvons dire que le couple Apache et Jakarta-Tomcat nous
permettent de faire le déploiement d'une application Web complet,
de plus ces outils sont gratuites.


\item [Couche Persistance de données.] Pour la couche de
persistance de données nous avons dit que nous allons profiter de
l'utilisation de JDBC (Java Database Connectivity). Le JDBC se
base sur l'utilisation d'un pilote pour donner l'accès à plusieurs
bases de données, l'avantage est que nous pouvons changer le
pilote selon le fournisseur de base de données sélectionné. Pour
le choix de la base de données nous avons dit que c'était ouvert à
l'implémentation, mais il faut toujours chercher les standards. Le
standard plus récemment publié par l'ANSI (American National
Standards Institute) pour le langage qui traite l'information
d'une base de données est SQL-99\footnote{
http:\//\//web.ansi.org}. Ce standard est utilisé par les plus
importants développeurs de bases de données comme Oracle\footnote{
http:\//\//www.oracle.com}, Informix\footnote{
http:\//\//www-306.ibm.com/software/data/informix/},
IBM-DB2\footnote{
http:\//\//www-306.ibm.com/software/data/db2/udb/},
PostgreSQL\footnote{ http:\//\//www.postgresql.org/}, et autres.
Pour PostgreSQL, il n'y a pas coût d'utilisation.\\

Tel que nous avons regardé dans cette section, nous profitons de
plusieurs composants ouverts qui nous permettent d'accélérer la
réalisation des systèmes tout en gardant un coût bas.
...
responsabilité la construction de l'horaire et non celle de la
vérification de données. Il est aussi plus facile de s'adapter au
changements.

D'autres bénéfices sont aussi à envisager, comme par exemple
consulter son horaire par le Web (à jour) depuis sa residence,
surtout dans des pays où parfois il fait $-30^\circ$ et moins, est
bien apprécié.
