...
develop software applications, the most adapted for the timetable
production system is that in layers.
...
exchanges with the interface and validate and check the data input
and sending the data to be presented in an adequate format. It
ensures also the exchanges with the data persistence  layer. The
...
earlier subsection, we propose a component which is part of the
...
a timetable. To enter the data, the user clerk has a Web page
which is displayed by a navigator, and it will have fields to
fill. At this moment, the interface layer can make some minimal
validations of the coherence of data, for example, check that the
data type is correct, to check that the fields which are
obligatory are filled, etc. Depending on the size of the
institution, it can have several clerks who work at the same time
in the application, it is necessary consequently, to take the
necessary measures to allow the concurrent work in the data
persistence layer.
...
data from our system, and they are sent to other systems or other
...
administration of the institution which must be informed of the
...
the maximum quantity of students in a group, all the courses of
mathematics are held the morning, etc.

\item The data results of these treatments must be store in a
place to be able to use them after its generation. It is the data
persistence layer which is responsible for this task. In a system
of timetable production, the data can be stored in a database
...
For the implementation of architecture, first of all it is
necessary, to find which of the technological elements we need.
The needs are:
...
never touching the element of the business logic. In the Table
...
to ensure a longer life the system.
...
for the timetable construction softwre showed us than~: the most
...
Then, the answer is yes, it exists a real benefit. By developing a
