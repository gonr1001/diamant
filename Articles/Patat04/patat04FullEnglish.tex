...
timetable for the courses and the exams per year. The automation
...
or more files, and their output is saved on one or more files too.
...
These kinds of errors are very hard to find, because all the data
...
in the software of construction, or in the software of production
or in both. The changes come on behalf of persons in charge for
the institution, for example a new nomenclature for the buildings;
a new teaching approach, which change the way to create the
groups, etc. The problem to modify software is not only for the
...
the PATAT conferences \cite{patat97,patat00,patat02}. On the other
hand, few efforts were devoted, until now, to the general
...
timetable construction. White \cite{white00} proposed that the Web
must be used to the diffusion of timetables. We are trying to go
far proposing also to use it like an interface of data input;
...
of Sherbrooke. We made an over flight of existing technologies in
...
pre-choice and others where the timetable is made without
...
coordinated. It is clear that the system must have two principal
...
both cases, the data sources are the database.
...
application that uses the data of the students is for example that
which print the notes of a student on his scorecard. Here, we are
...
data, by example, the format of a course, which pass from 3h
...
to save the changes carried out for iterations and to make a
...
In an abstracted way, software architecture describes the
component elements of a system; it shows also the interactions
between these elements, the models that guide its composition and
...
solutions together, we will arrive at the solution of our complex
...
\begin{description}
\item [The interface.] It is responsible to present the data to
the user, to allow the data input, and ensuring the exchanges with
other systems.

\item [Business logic layer.] It is responsible to ensure the
exchanges with the interface layer. Validate and check the data
input and sending the data to be presented in an adequate format.
It ensures also the exchanges with the data persistence layer. The
business rules are used to ensure the coherence of the system.
...
timetable production and suggested architecture.
...
\begin{enumerate}
\item We defined three categories of the data in a timetable
production system. The figure \ref{fig:fTwo} shows these
categories. The interface layer treats these data as follows:

\begin{enumerate}

\item Basic data. These data are in a database belonging to the
...
\item Direct data entry of the one period $P$. This activity is
made by a clerk. Normally, these data change with each creation of
a timetable. To enter the data, the user clerk has a Web page that
is displayed by a navigator, and it will have fields to fill. At
this moment, the interface layer can make some minimal validations
of the coherence of data, for example, check that the data type is
correct, to check that the obligatory fields are filled, etc.
Depending on the size of the institution, it can have several
clerks who work at the same time in the application, it is
necessary consequently, to take the necessary measures to allow
the concurrent work in the data persistence layer.
...
nature, it can have a certain number of versions of the work that
...
version, he must apply the operation ''commit'' to make available
the information to the rest of the users. Those are the output
data from our system. They are sent to other systems or other
users, for example, the students who will receive a paper with
their personalized timetable or he can consult this information by
using a Web site creates for these ends, or, the central
administration of the institution that must be informed of the
...
able to express constrains, for example, the maximum quantity of
students in a group, all the courses of mathematics are held the
morning, etc.

\item The data results of these treatments must be store in a
place to be able to use them after its generation. The data
persistence layer is responsible for this task. In a system of
...
find which of the technological elements we need. The needs are:
...
At the first, it should be known that an element is open if it is
built with approved standards, or, if it is built with a private
...
enter data we chose HTML and JSP, given that those languages are
the standard for the creation of Web pages, and we want to benefit
...
they can be the object of requests and treated like any other data
format.
...
with the minimum of redundancy. These data must be used by
programs written in different languages by using standard
technology ODBC (Open DataBase Connectivity), if we use Java as
language to establish the connection to the database, we can use
JDBC (Java Database Connectivity). The standard JDBC is an
...
timetable construction profit from developments associated with
the timetable production? Our experience in software development
for the timetable construction software showed us than~: the most
of the problems came from the fact that the input data has errors.
The construction software must dedicate a large part of its code
to detect its errors. Moreover, users take an important time to
check the coherence of data before beginning the building work.
Then, the answer is yes, it exist a real benefit. By developing a
system of production, it is easier to ensure than the software of
construction has the responsibility for timetable construction and
not the responsibility of checking the data.
...
in countries where sometimes it makes $-30^\circ$, that is well
appreciated.
