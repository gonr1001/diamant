...
the timetable construction, we mention DIAMANT
\cite{gonzalezrubio00} and SAPHIR \cite{ferland94}, but, there
exists many others.

In general, the timetable creation software uses one or more files
as input, and one or more files as output. Working with files is a
problems source. It is necessary to build the input files each
time that a timetable is produced. There exists, two cases~: a
manual production and an automated production of those files. If
the files are manually produced, there are the typing errors that
can produce unexpected effects in the software. These kinds of
errors are very hard to detect, because all the data should be
checked manually. In the case where the files are produced in an
automatic way, it is necessary that the software be ensured to
check the validity and the coherence of all the data.
...
data checking and data validation at the time of acquisition. In
order to ensure that the software of construction works with the
right data.
...
We presented the architecture of a schedules production system in
...
timetable on the Web from the user's home. Especially in countries
