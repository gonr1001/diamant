...
\title{\bf{The architecture of a system of timetable production of courses and exams}}
...
exists, two cases~: a manual production of the files or an
automated production. If the files are produced manually, there
are the typing errors that can produce unexpected effects in the
software. The errors are hard to find, because all the data should
be checked manually. In the case, where the files are produced in
an automatic way, it is necessary that the software is ensured to
check the validity and the coherence of all the data. The second
problem appears when a change is introduced into the system
(either in the application of construction, or in that who produce
the files) or if the application of construction is used in
several different sites, using different data formats.
...
the timetable production. It acts of a general problem in the
...
timetable production of courses and exams having the qualities:
...
software of timetable construction. The checking and the
...
to offer the timetable in a personalized way, once that it is
built. The database allows the retrieval of the data, according to
various formats. Thus, the input of the software of construction
is simplified. Of course, with the assistance of the software of
timetable construction, it is possible to create a timetable with
...
having a total sight on the process can help the research in
...
data format, in the many instances of software for timetable,
their goal was to compare algorithms (programs) in benchmarks. In
a more recent work, Kingston \cite{kingston02} proposes a language
STTL and an interpreter, with the same objective. Özcan
\cite{ozcan03} suggests also another standard to define instances
of problems of timetable construction, having XML as a base
language. It seems that it would be desirable to have a standard,
but it is a goal very hard to reach. At the place, our approach
makes it possible to use a database like a tool of extraction and
formatting, in this way the software of timetable construction can
receive the necessary data. Thus, it would be possible to generate
other instances of problems for other software of timetable
construction.

The development of our architecture, was divided into two parts,
we analyzed the flow of the data in a generic system of timetable
...
\section{The flow of information in the timetable production}
\label{sec:data}

First of all, we define, the process of timetable production as a
process which starts with the data capture for the period of
...
for example the courses which will take place during the period
$P$; the format of the courses (3h, or 2h+1h); possible or
assigned teachers for certain courses; possible or assigned
buildings; the availability of the teachers and the buildings,
...
coordinated. It appears clear to us that the system must have two
principal components~: the system of data back-up and the software
of timetable construction.

\item[Data] represent the data necessary to the timetable
...
For example, the means of acquisition of pre-choice, will save the
courses followed by a student. The module of construction of data
will take its entry in the database and when the construction is
made the new data obtained will be given in the data warehouse .


\item[Paper or Web] It is the output of the system once that the
timetable is built. These output can to be files, which will be
printed on paper or data that will be presented on a Web page, in
both cases the data sources are in the database.
...
the data at differents moments of the process.

The distribution of data in only one database or several data
bases (distributed database) is a problem of implementation. In a
conceptual way it is only one database.
...
\put(0, 15){\fichier[$bd_{1}$]} \put(0, 5){\makebox(20,10){Basic
data}} \put(20, 20){\vector(1,0){10}} \put(30,
15){\fichier[$bd_{2}$]} \put(30, 5){\makebox(20,10){Period $P$
data}} \put(50, 20){\vector(1,0){10}} \put(60,
15){\fichier[$bd_{3}$]} \put(60, 5){\makebox(20,10){Timetable
data}}
\end{picture}%}
\end{center}
...
the warehouse. The information of the academic activities of a
program, the programs offered, the teachers, the characteristics
of classrooms, the students, all of them are examples of this type
of information. In general this information is shared by other
applications. For example a student is described by his first
name, last name, number identification, program, etc. An
...
which can be a sight (partially) of the complete institutional
database.
...
timetable construction and are captured before each construction.
However, these data will change (update) during the timetable
construction. For example when a teacher external is assigned for
a given course, we will enter his information name and
...
print or show general and personalized timetable.

\end{description}

It should be noted that there are two great activities in the
production~: the data acquisition (updating) and timetable
construction. In general these two activities are carried out by a
group of people. It is also necessary to see how these activities
...
arrives in very rare cases. In the case B the data input
(especially the update) is done at the same time as the
construction. This is the general case, much of data are fixed, as
the offer of course, however, during the construction we find
situations where the conflicts oblige to make changes on the data,
the format of a course, who pass from 3h consecutive to 2h
consecutive more 1h. Other cases can be presented.

According to each instance of the system, it is necessary to
define the schema of the database, and the roles and privileges of
the users.

Another dimension of these two activities is the interaction of
the users with the databases. We consider two types of users~: the
clerk and the person in charge of construction. The clerk is that
which will enter the basic date, the person in charge of
construction which is responsible for the construction of the
timetable and which interacts with the timetable construction
program.

The clerk will enter or update the data in the database if the
data is valid, it is accepted and entered the base. For example if
the data waited is an integer dans un interval, and the input data
correspond with the attended set of values, it will be entered to
the database. If not, the clerk must return with a new value. Of
course, it is necessary to have various priorities of access.


The timetable construction is an activity of an iterative nature.
...
will try to find several solutions to choose one. In certain
cases, it is possible to leave the program with the same data to
find a new solution. In other cases it is necessary to change the
starting data to obtain a new solution. If it is necessary to make
$n$ tests the solution chosen by the person in charge will be the
solution $s$ where $1 \leq s \leq n$. It is necessary to envisage
a way of working with the database in order to save the changes
carried out for each iteration and to make a ``commit' ' when the
solution is chosen and that the timetable becomes final for the
period.
