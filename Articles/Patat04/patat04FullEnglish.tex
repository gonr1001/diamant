...
schedules per year for the courses and the exams. The automation
...
construction of schedules; in others, it starts with the
automation of the treatments associated with the work of the
institution, as for example the follow of students. There are
software to carry out the construction of schedules, by example
DIAMANT \cite{gonzalezrubio00} and SAPHIR \cite{ferland94}, but,
there exists many others.

In general, the software for the creation of schedules uses as
input one or more files. Their output is saved on one or more
...
buildings; new teaching approach, which change the way to create
the groups, etc. The problem to modify a software is not only for
the production of schedules. It acts of a general problem in the
...
production of schedules of courses and exams having the qualities:
open and extensible. This architecture must contribute to more
easily work with the two problems mentioned, by reducing the cost
of the modifications and by carrying out the checking and the
validation of the data in precise stages of the process and by
avoiding the redundancy of the code and the data. This
architecture implements a database, an Internet site and a
software of construction of schedules. The checking and the
validation of data will be made with the input and with integrity
constraints in the database. The Internet site is used mainly for
the data input, at the same time, it can check the validity of the
data, using the rules of businesses before the data will be
entered to the database. Another function of the Internet site is
to offer the schedules in a personalized way, once that the
schedule is built. The database allows the retrieval of the data,
according to various formats. Thus, the input of the software of
construction is simplified. Of course, with the assistance of the
software of construction of schedules, it is possible to create a
schedule with a minimum of conflicts.


Many research was made in the construction of schedules:
algorithms, mathematical formulations and several software were
proposed in the PATAT conferences \cite{patat97, patat00,
patat02}. On the other hand, few efforts were devoted, until now,
to the general development of the production of schedules. We
think that the fact of having a total sight on the process can
help the research in construction of schedules. White
\cite{white00} highlighted that the Web must be used to the
diffusion of schedules. We are trying to go far proposing also to
use it like interfaces of data input, moreover we propose the
architecture necessary to support our ideas. De Causmaecker et al.
\cite{decausmaecker02} introduced the idea that the semantics Web
can be used by the researchers in the construction of schedules.
They also introduced the idea to use XML like a language of
communication in the construction of schedules by agents. We
propose also the use of XML, in order to facilitate the exchanges
between certain modules of our architecture. In Burke and Al
\cite{burke97} it is proposed to have a standard for the data
format, in the many software for schedule instances, their goal
was to compare algorithms (programs) in benchmarks. In a more
recent work, Kingston \cite{kingston02} proposes a language STTL
...
construction of schedules, having XML as a base language. It seems
that it would be desirable to have a standard, but it is a goal
very hard to reach. At the place, our approach makes it possible
to use a database like a tool of extraction and formatting, in
this way the software of construction of schedules can receive the
necessary data. Thus, it would be possible to generate other
instances of problems for other software of construction of
schedules.

The development of our architecture, was divided into two parts,
we analyzed the flow of the data in a generic system of production
of schedules. Then we define an architecture, the basic blocks and
...
establish it in a system of production of schedules at the
University of Sherbrooke. We made an overflight of existing
...
\section{The flow of information in the production of schedules}
\label{sec:data}

First of all, we define, the process of production of schedules as
a process which starts with the data capture for the period of
validity of the schedule $P$. This period $P$ is one six-month
period, one quarter, or a year\footnote{In the University of
Sherbrooke we speaks about quarter, but it really comprises
sixteen weeks (almost four months), therefore the system must
adapt to any period $P$.}. The process ends when all the concerned
people receive the schedule on a paper or the Web (see figure
\ref{fig:fOne}) and that the period of validity of the schedule
touches at its end. The are multiple data to feed to the system,
for example the courses which will take place during the time $P$;
the format of the courses (3h, or 2h+1h); possible or assigned
teachers for certain courses; possible or assigned buildings; the
availability of the teachers and the buildings, etc. We know that
there are institutions where the students make a pre-choice and
others where the schedules are made without pre-choice. The
pre-choice of the students is part of the data to be supplied, if
there is no pre-choice, it is necessary to indicate the number of
students to be admitted for each course.
...
\put(35, 12){\process[Schedule\\production\\software]}
...
\end{picture}
\end{center}
\caption{Production of schedules}\label{fig:fOne}
\end{figure}

The figure \ref{fig:fOne} is an abstract view of the production of
schedules, the system of production includes a software of
