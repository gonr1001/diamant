...
Le dialogue d'affectation d'un événement doit être modifié de
sorte à rajouter l'état et la fonction du local (voir figure
\ref{dlg_event}).
...
Le dialogue d'option de conflits doit être modifié de sorte à
rajouter le taux de remplissage des locaux (voir figure
\ref{dlg_conf}).

\begin{figure*}[h]
  % Requires \usepackage{graphicx}
  \begin{center}
    \includegraphics[width=3.0in]{SDD/images/dlg_conflicts.eps}
...
\begin{itemize}
    \item \verb!dInterface.dUtil.ConflictDlg!: le dialogue
    permettant de spécifier les contraintes d'optimisation.
\end{itemize}

\subsubsection{Algorithme d'affectation de locaux}

L'affectation de locaux est un problème d'optimisation, dans
lequel on cherche à construire une solution à un problème qui
optimise une fonction objectif. L'algorithme glouton utilisé pour
résoudre ce problème est le suivant:
\begin{verbatim}
    //on va construire la solution dans Sol
    //initialisation
    Ens= E;/* ensemble des locaux disponibles pour
    une affectation: DSetOfResources*/
    ...
    retourne Sol
    \end{verbatim}
La nouvelle classe à créer pour l'implémentation de cet algorithme
sera\\ \verb!dInternal.dOptimization.RoomAssignmentAlgo!. La
structure de cette classe est présentée à la figure \ref{algo}.

\begin{figure*}[h]
  % Requires \usepackage{graphicx}
  \begin{center}
    \includegraphics[width=2.0in]{SDD/images/roomassignmentalgo.eps}
    \caption{Structure de la classe RoomAssignmentAlgo}\label{algo}
  \end{center}
\end{figure*}
...
Les champs et méthodes de la classe \verb!RoomAssignmentAlgo!
auront les responsabilités:

\begin{description}
    \item[ensE :] c'est l'ensemble fini de locaux pouvant être
    affectés.
    \item[solution :] c'est l'ensemble des solutions du problème.
    Chaque solution est un couplet (local, événement).
    \item[select():] elle permet de choisir le meilleurs local parmi
    les locaux restant à affecter, en respectant les contraintes de capacité et de
    fonction des locaux.
    \item[isFinish():] elle teste si une solution partielle est
    une solution complète.
    \item[isAddPossible():] elle teste si un couplet (local,
    événement)peut être ajouté à une solution partielle.
    \item[add():] elle permet d'ajouter un couplet (local,
    événement) à une solution partielle si c'est possible.
    \item[build():] c'est l'algorithme à proprement parler, il est
    basé sur un critère local de sélection des éléments de ensE
    pour construire une solution optimale.
\end{description}
