...
\subsubsection{Description du sous-composant ttstructure}
 ...
 grille horaire. La structure de ce sous-composant est de type hiérarchique et le noeud principal est appelé ttstructure. Une ttstructure peut être composée
 ...
 périodes. La période est la plus petite unité d'une ttstructure,
 elle est décrite en minutes.

 \todo{Placer une image de la ttstructure}
...
d'entrée (fenêtre principale - \emph{main display}). L'interface
principale intègre trois composants: la barre de menu (\emph{menu
bar}), la fenêtre interne (\emph{internal frame}) et la barre
d'état (\emph{state bar}).
...
\begin{description}
    \item[Command: ] elle déclare une interface permettant
    d'éxécuter une opération. Elle gardera le même nom dans
    \diaman{}.
    \item[Invoker: ] elle externalise la requête de la commande.
    Elle sera identifiée dans \diamant{} par la classe CmdMenu.
    \item[ConcreteCommand: ] elle implemente la methode Execute
...
La barre d'état permet de visualiser à chaque instant l'état de
certaines informations (les enseignants; les locaux, les
activitées, les étudiants, les événements et les confits).

\subsection{Fenêtre interne}

La fenêtre interne est le corps de l'interface graphique. Elle
présente une grille horaire pouvant contenir des événements

 \todo{Placer une image de l'organisation de la vue}
