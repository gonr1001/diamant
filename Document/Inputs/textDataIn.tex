...
Les  versions 1.0, 1.5 et 1.6 de \diamant{} utilisent les fichiers
suivantes~:
...
%-----------------------------------------------------------------------------
\section{Fichier d'étudiants version 1.5}
...
%-----------------------------------------------------------------------------
\section{Fichier d'étudiants version 1.6}
...
\begin{enumerate}
    \item Ligne d'identification de la version de Diamant (Ligne de
    type 1).
    \item Ligne de type E qui contient l'identification de
    l'étudiant (Ligne de type 2).
    \item Ligne de type C qui contient l'identification des cours que l'étudiant suit (Ligne de
    type 3).
    \item Ligne de type T qui contient le nombre d'étudiants dans le
    fichier (Ligne de type 4).
\end{enumerate}

Voici la structure du fichier.

\subsubsection{Détails de la ligne de type 1}

Cette ligne contient le nom de la version de Diamant pour laquelle
le fichier a été crée, dans l'exemple:

Diamant1.6


\subsubsection{Détails de la ligne de type 2}

\begin{itemize}
      \item Cette ligne commence avec la lettre E (Indice 1 à 1) et un espace (Indice 2 à 2).

      \item Le matricule de l'étudiant qui correspond aux huit premiers caractères
      du numéro d'identification (Indice 3 à 10). Dans notre exemple, \verb!00916487! pour l'étudiante
      \verb!Gagnon-Auger, Maude! et \verb!00928438! pour l'étudiante \verb!Rivest, Catherine!.

      \item Le programme auquel l'étudiant appartient correspond ensuite aux
      six caractères suivants.
      Soit \verb!260500! dans le cas du premier étudiant (Indice 11 à 16).

      \item L'instance du programme que l'étudiant suit correspond aux deux
      caractères suivants. Soit \verb!02! dans le cas du premier étudiant (Indice 17 à 18).

      \item Des informations sur l'admission de l'étudiant sont données
      par les cinq caractères suivants. Les quatre premiers correspondent à l'année d'admission tandis
      que le cinquième indique le trimestre d'admission. Soit \verb!20043! dans le cas du premier étudiant (Indice 19 à 24).

      \item Un espace vide (Indice 24 à 24)

      \item Suivent le nom de famille de l'étudiant suivi d'une virgule et de son prénom (Indices 25 à 29XXXXX)
      \corrkad{ 29XX ou bien jusqu`à la fin du nom de l'étudiant}.
      Soit, dans notre exemple, \verb!Gagnon-Auger, Maude! dans le cas de la première étudiante.
\end{itemize}

\subsubsection{Détails de la ligne de type 3}

La ligne de type 3 contient les activités suivies par l'étudiant, un
activité par chaque ligne.

Les détails de la ligne de type 3.

\begin{itemize}
      \item Cette ligne commence avec la lettre C (Indice 1 à 1) et un espace (Indice 2 à 2).

     \item Le siegle de l'activité pédagogique (\verb!KIN102!) (Indice 3 à 8).

     \item Un caractère qui correspond à la nature ou type d'activité du cours (\verb!1!)
     (1=Leçon Magistrale et 2=autre)(Indice 9 à 9).

     \item Le groupe d'activité dans lequel l'étudiant est assigné. Au départ ils sont
     (\verb!00!). (Indice 10 à 11).

     \item Un espace vide (Indice 12 à 12).

     \item Trois lettres qui représentent le site où l'activité en question est
     donnée, dans l'exemple (\verb!SHE!) (SHE = Sherbrooke, LON =
     Longueil, COW = Cowansville)
...
\subsubsection{Détails de la ligne de type 4}

\begin{itemize}
      \item Cette ligne commence avec la lettre T (Indice 1 à 1) et un espace (Indice 2 à 2).

     \item Le total des étudiants envoyés dans le fichier (Indice 3 à 7).
...
%-----------------------------------------------------------------------------
\section{Fichier d'activités version 1.5}
...
%-----------------------------------------------------------------------------
\section{Fichier d'activités version 1.6}
...
Il existe quatorze types de lignes :
...
\begin{enumerate}
\item Le fichier commence par une ligne qui contiens la version du
logiciel, dans l'exemple Diamant1.6

\item vide, il s'agit d'une ligne du premier type.
\item Ensuite il y a $n$ couples de lignes allant du type 3 au type 14.
    \begin{enumerate}
    \item La ligne de type 2 est une chaîne de caractères correspondant au sigle
    du cours (\verb!EPD100!) (Indice 1 à 6).

    Ceci décrit la leçon magistrale \verb!1!  du cours \verb!EPD100!
    (Indice 7 à 7). E le groupe \verb!01!) (Indice 10 à 11).

    Le site de l'activité \verb!SHE!) (Indice 13 à 15) (SHE = Sherbrooke, LON =
     Longueil, COW = Cowansville).

     La capacité du groupe (Indice  20 à 21)

    Dans l'exemple cette ligne contient \verb!EPD1001  01 SHE    22!
    ...
    Dans l'exemple cette ligne contient \verb!1!
    \item La ligne de type 5 est une chaîne de caractères correspondant au nom de l'enseignant.
    Dans l'exemple cette ligne contient \verb!ST-ARNAUD, L.!
    \item La ligne de type 6  le nombre de blocs de périodes séparées de la première fiche
    (ex: si nous avons deux heures de cours collées à
    trois endroits différents dans l'horaire, l'entier sera 3).
    Dans l'exemple cette ligne contient \verb!2!
