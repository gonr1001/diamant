...
Le présent document contient les exemples, les interprétations et l'énumération des différentes
informations requises par \diamant{} pour la construction de
l'horaire.
...
\texttt{001342\\
00900813 203503 07 20003LUPIEN MY05\\
...
\begin{enumerate}
\item Le chiffre du début du fichier, \texttt{001342}, nous indique
le nombre d'étudiants contenus dans ce fichier, soit le nombre
d'inscriptions jusqu'à présent.
\item La ligne de chiffres et de lettres correspond au numéro
d'identification unique de l'étudiant (à l'exception des deux dernier entiers de la ligne).
Ce numéro d'identification renferme plusieurs informations
pertinentes. Soit :
\begin{itemize}
      \item Le matricule de l'étudiant qui correspond aux huit premiers chiffres du numéro d'identification. Dans notre exemple, \texttt{00900813} pour l'étudiant \texttt{LUPIEN} et \texttt{00901199} pour l'étudiante \texttt{AUDET}.
      \item Le programme auquel l'étudiant appartient correspond ensuite aux six chiffres suivants. Soit \texttt{203503} dans le cas du premier étudiant.
     \item La version du programme que l'étudiant suit correspond aux deux chiffres suivants. Soit \texttt{07} dans le cas du premier étudiant.
      \item Des informations sur l'admission de l'étudiant sont données par les cinq chiffres suivants. Les quatre premiers correspondent à l'année d'admission tandis que le cinquième indique le trimestre d'admission. Soit \texttt{20003} dans le cas du premier étudiant.
    \item Suit ensuite neuf places pour indiquer le nom de famille de l'étudiant suivit d'un espace et de son prénom (coupé après neuf espaces). Soit, dans notre exemple, \texttt{LUPIEN MY} dans le cas de la première étudiante.
\end{itemize}

\item Vient alors un chiffre de deux caractères qui correspond en fait au nombre de cours que suit l'étudiant. \texttt{05} dans le cas du premier étudiant.
\item Finalement, nous avons la liste des activités suivies par l'étudiant. Soit \texttt{CTB301101} dans le cas du premier étudiant. Les 6 premiers caractères (\texttt{GEI441}) correspondent au nom du cours. Le septième caractère (\texttt{1}) correspond à la nature du cours (1=Leçon Magistrale et 2=autre). Le 2 derniers caractères (\texttt{01}) sont facultatifs dans un fichier et ils correspondent au groupe d'activité dans lequel l'étudiant est assigné
\end{enumerate}
...
dans la journée (8h30, 9h30, 10h30, 11h30, 12h30,13h30, 14h30, 15h30, 16h30, 17h30, 18h30, 19h00, 20h00, 21h00 et 22h00). Les disponibilités sont ainsi connues entre 8h30 et 21h30.
...
activités associées, l'agencement du cours, le nom de l'enseignant qui le donnera, etc. C'est un fichier préalable à la construction de l'horaire.
...
\begin{description}
    ...
    \begin{itemize}
        ...
        \item La fonction du local \texttt{211} pour le premier local (voir le manuel d'utilisation de \saphir{} \cite{ruben94})
        \item Liste des caractéristiques 08,11,14,57 pour le premier local (voir le manuel d'utilisation de \saphir{} \cite{ruben94}). Il peut y avoir plusieurs caractéristique, elles seront séparées par des \texttt{,}
