...
public class ConditionsTest extends TestCase {
	...
	public void test_getConflictsEventInTTs5j() {
		...
		((EventAttach) event.getAttach()).setAssigned(true);
	...
	public void test_buildAllConditions() {
		...
		Period per = _dmh.getTTStructure().getCurrentCycle().getPeriodByKey(3, 1,
				2);
	...
	public void test_buildAllConditionsInCloneTTS() {
		...
		assertEquals("test1_buildAllConditionsInCloneTTS : assertEquals 1", 2, per
				.getNbStudConflict());
		assertEquals("test2_buildAllConditionsInCloneTTS : assertEquals 2", 0, per
				.getNbInstConflict());
		assertEquals("test3_buildAllConditionsInCloneTTS : assertEquals 3", 0, per
				.getNbRoomConflict());
		// get period day=1 (lundi), sequence = 3 (PM) and period = 6 (15h30)
		per = cloneTTS.getCurrentCycle().getPeriodByKey(1, 3, 6);
		assertEquals("test4_buildAllConditionsInCloneTTS : assertEquals 4", 1, per
				.getNbStudConflict());
		assertEquals("test5_buildAllConditionsInCloneTTS : assertEquals 5", 2, per
				.getNbInstConflict());
		assertEquals("test6_buildAllConditionsInCloneTTS : assertEquals 6", 0, per
				.getNbRoomConflict());
		boolean isEquals = _dmh.getTTStructure().isEquals(cloneTTS);
		assertEquals("test7_buildAllConditionsInCloneTTS : assertEquals 7", true, isEquals);
		...
		assertEquals("test8_buildAllConditionsInCloneTTS : assertEquals 8", 14, per
				.getNbStudConflict());
		assertEquals("test9_buildAllConditionsInCloneTTS : assertEquals 9", 1, per
				.getNbInstConflict());
		assertEquals("test10_buildAllConditionsInCloneTTS : assertEquals 10", 0, per
				.getNbRoomConflict());
		...
		assertEquals("test11_buildAllConditionsInCloneTTS : assertEquals 11", 40, per
				.getNbStudConflict());
		assertEquals("test12_buildAllConditionsInCloneTTS : assertEquals 12", 0, per
				.getNbInstConflict());
		assertEquals("test13_buildAllConditionsInCloneTTS : assertEquals 13", 0, per
				.getNbRoomConflict());
		per = cloneTTS.getCurrentCycle().getPeriodByKey(2, 1, 3);
		assertEquals("test14_buildAllConditionsInCloneTTS : assertEquals 12-1", 1, per
				.getNbInstConflict());
...
//		// get period day=3 (mercredi), sequence = 1 (AM) and period = 2 (8h30)
//		Period per = cloneTTS.getCurrentCycle().getPeriodByKey(3, 1, 2);
//		assertEquals("test1_buildAllConditionsInCloneTTS7j : assertEquals 1", 2, per
//				.getNbStudConflict());
//		assertEquals("test2_buildAllConditionsInCloneTTS7j : assertEquals 2", 0, per
//				.getNbInstConflict());
//		assertEquals("test3_buildAllConditionsInCloneTTS7j : assertEquals 3", 0, per
//				.getNbRoomConflict());
//		// get period day=1 (lundi), sequence = 3 (PM) and period = 6 (15h30)
//		per = cloneTTS.getCurrentCycle().getPeriodByKey(1, 3, 6);
//		assertEquals("test4_buildAllConditionsInCloneTTS7j : assertEquals 4", 1, per
//				.getNbStudConflict());
//		assertEquals("test5_buildAllConditionsInCloneTTS7j : assertEquals 5", 2, per
//				.getNbInstConflict());
//		assertEquals("test6_buildAllConditionsInCloneTTS7j : assertEquals 6", 0, per
//				.getNbRoomConflict());
//		boolean isEquals = dm.getTTStructure().isEquals(cloneTTS);
//		assertEquals("test7_buildAllConditionsInCloneTTS7j : assertEquals 7", true, isEquals);
//
...
//				new DResource("PED200.1.21.1.", eventClone), false);
//		per = cloneTTS.getCurrentCycle().getPeriodByKey(1, 1, 2);
//		assertEquals("test8_buildAllConditionsInCloneTTS7j : assertEquals 8", 14, per
//				.getNbStudConflict());
//		assertEquals("test9_buildAllConditionsInCloneTTS7j : assertEquals 9", 1, per
//				.getNbInstConflict());
//		assertEquals("test10_buildAllConditionsInCloneTTS7j : assertEquals 10", 0, per
//				.getNbRoomConflict());
//
//		// set event key tuesday at 8h30
//		eventClone = ((EventAttach) event.getAttach()).cloneEvent();
...
//				new DResource("PED200.1.21.1.", eventClone), false);
//		per = cloneTTS.getCurrentCycle().getPeriodByKey(2, 1, 2);
//		assertEquals("test11_buildAllConditionsInCloneTTS7j : assertEquals 11", 40, per
//				.getNbStudConflict());
//		assertEquals("test12_buildAllConditionsInCloneTTS7j : assertEquals 12", 0, per
//				.getNbInstConflict());
//		assertEquals("test13_buildAllConditionsInCloneTTS7j : assertEquals 13", 0, per
//				.getNbRoomConflict());
//		per = cloneTTS.getCurrentCycle().getPeriodByKey(2, 1, 3);
//		assertEquals("test14_buildAllConditionsInCloneTTS7j : assertEquals 12-1", 1, per
//				.getNbInstConflict());
//		//set event key wedneday at 15h00
//		eventClone.setKey(4, "3.3.5");
//		dm.getConditionsTest().addEventInTTs(cloneTTS,
//				new DResource("PED200.1.21.1.", eventClone), false);
//		per = cloneTTS.getCurrentCycle().getPeriodByKey(3, 3, 5);
//		assertEquals("test15_buildAllConditionsInCloneTTS7j : assertEquals 14", 84, per
//				.getNbStudConflict());
//		assertEquals("test16_buildAllConditionsInCloneTTS7j : assertEquals 15", 0, per
//				.getNbInstConflict());
//		assertEquals("test16_buildAllConditionsInCloneTTS7j : assertEquals 16", 0, per
//				.getNbRoomConflict());
//		per = cloneTTS.getCurrentCycle().getPeriodByKey(3, 3, 7);
//		assertEquals("test16_buildAllConditionsInCloneTTS7j : assertEquals 14", 2, per
//				.getNbStudConflict());
//	}
//	
