...
/*
  ...
  * */
  ...
  private int partitionID(int begin, int end){
   ...
   while( i< j){
     i++;
     while(((DResource)_resourceList.get(i)).getID().compareTo(pivot.getID()) < 0)
       i++;
     j--;
     while(((DResource)_resourceList.get(j)).getID().compareTo(pivot.getID()) > 0)
       j--;
     if(i < j)
       swap(i,j);
   }
  ...
  private int searchKey(long key){
    ...
    while(low <= high){
      int mid = (low + high)/2;
      int diff=1;
      middleKey = ((DResource)_resourceList.get(mid)).getKey();
      if (middleKey> key)
        diff = 1;
      else{
         if (middleKey< key)
  ...
  private int searchID(String id){
    ...
    while(low <= high){
      int mid = (low + high)/2;
      int diff = ((DResource)_resourceList.get(mid)).getID().compareTo(id);
      if (diff == 0)
        return mid;
      //else{
        if (diff < 0)
