  ...
  */
  ...
  private String getReport(String allLines, int principalAct, int[] otherAct){
    ...
    for (int i=0;i< setOf.size(); i++){
      ...
      Vector vec= (Vector)((DValue)setOf.getResourceAt(i).getAttach()).getObjectValue();
  ...
  private DSetOfResources createSortReport(DSetOfResources setOr, String line){
    ...
    if(index==-1){
      setOr.addResource(new DResource(firstToken,new StandardCollection()),1);
      createSortReport(setOr,line);
    }else{// end if(index==-1)
      String newLine= DXToolsMethods.removeToken(line,";",0);
      DSetOfResources sor= (DSetOfResources)setOr.getResourceAt(index).getAttach();
   ...
   * @return
   */
  private void writeSortReport(DSetOfResources setOr, String line, Vector result){
    //="";
    if(setOr.size()!=0){
      for(int i=0; i< setOr.size(); i++){
        //line+= setOr.getResourceAt(i).getID()+";";
        DSetOfResources sOr = (DSetOfResources)setOr.getResourceAt(i).getAttach();
