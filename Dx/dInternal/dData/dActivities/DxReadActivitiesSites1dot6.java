 ...
 * 
 * Project Dx
 * Title: DxReadActivitiesSites1dot6.java 
 * 
 *
...
public class DxReadActivitiesSites1dot6 implements DxActivitiesSitesReader {
	...
	public DxReadActivitiesSites1dot6(DataExchange de,
	...
	public DxSetOfActivitiesSites readSetOfActivitiesSites() throws Exception {
		...
		StringTokenizer st = new StringTokenizer(_deActivities.getContents(),
				"\r\n");
		st.nextToken(); // Skips the Diamant 1.6 line
		...
		DxActivitySite dxasCurentSite = null;
		...
		while (st.hasMoreElements()) {
			...
			case 0:// activity name
				...
				if (stLine.countTokens() != DConst.NUMBER_OF_TOKEN_COURSE_LINE) {
					// TODO: Must throw error
					// _error = DConst.ACTI_TEXT3 + line;
				}

				String sActivity = stLine.nextToken();
				if (sActivity.trim().length() < DConst.SIZE_OF_COURSE_TOKEN) {
					// TODO: Must throw error
					// _error = DConst.ACTI_TEXT1 + line;
				}
				String sActivityName = sActivity.substring(0, _COURSENAMELENGTH);
				String sTypeName = sActivity.substring(_COURSENAMELENGTH,
						_COURSENAMELENGTH + 1);
				String sSectionName = stLine.nextToken();
				if (sSectionName.length() != DConst.SIZE_OF_GROUP_TOKEN) {
					// TODO: Must throw error
					// _error = DConst.ACTI_TEXT14 + line;
				}

				try {
					Integer.parseInt(sSectionName);
				} catch (NumberFormatException e) {
					// TODO: Must throw error
					// _error = DConst.ACTI_TEXT14 + line;
				}

				String sSiteName = stLine.nextToken();
				if (sSiteName.length() != DConst.ACT_SITE_LENGTH) {
					// TODO: Must throw error
					// _error = DConst.ACTI_TEXT15 + line;
				}
				dxasCurentSite = dxsoasAllSites.getActivitySite(sSiteName);
				if (dxasCurentSite == null) {
					dxasCurentSite = new DxActivitySite(sSiteName);
				...
				if (dxsSection == null) {
					dxsSection = new DxSection(sSectionName);
					dxtType.addSection(dxsSection);
				}

				String sCapacity = stLine.nextToken();
				if (sCapacity.length() != DConst.ACT_CAPACITY_LENGTH) {
					// TODO: Must throw error
					// _error = DConst.ACTI_TEXT16 + line;
				}
				int nCapacity = 0;
				try {
					nCapacity = Integer.parseInt(sCapacity);
				} catch (NumberFormatException e) {
					// TODO: Must throw error
					// _error = DConst.ACTI_TEXT16 + line;
				}
				dxaActivity.setCapacity(nCapacity);
			...
			case 8:// Preferred rooms
				...
				while (iUnities.hasNext()) {
					...
					if (dxrRoom == null) {
						// TODO: Must throw error
						// Should throw an excpetion as this room does not
						// exist
					}
