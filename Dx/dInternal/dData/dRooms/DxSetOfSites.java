...
//	}
	...
	public void addRoom(String sSiteName, String sCatName, DxRoom dxrRoom) {
		try {
			((DxSite) this.getResource(sSiteName)).addRoom(sCatName, dxrRoom);
		} catch (Exception e) {
			// Could produce a null pointer exception if get returns null
		}
	...
	public DxCategory getCat(String sSiteName, String sCatName) {
		try {
			return ((DxSite) this.getResource(sSiteName)).getCat(sCatName);
		} catch (Exception e) {
			// if key was invalid, getter returns null
			return null;
		}
	...
	public long getCatKey(long lSiteKey, String sCatName) {
		try {
			return ((DxSite) this.getResource(lSiteKey)).getCatKey(sCatName);
		} catch (Exception e) {
	...
	public DxRoom getRoom(String sSiteName, String sCatName, String sRoomName) {
		try {
			return ((DxSite) this.getResource(sSiteName)).getRoom(sCatName,
					sRoomName);
		} catch (Exception e) {
			// if key was invalid, getter returns null
			return null;
	...
	public long getRoomKey(long lSiteKey, long lCatKey, String sRoomName) {
		try {
			return ((DxSite) this.getResource(lSiteKey)).getRoomKey(lCatKey,
					sRoomName);
		} catch (Exception e) {
			// If index was invalid, Null pointer Exception will be thrown
			return -1;
	...
	public int getRoomCapacity(String sSiteName, String sCatName,
			String sRoomName) {
		try {
			return ((DxSite) this.getResource(sSiteName)).getRoomCapacity(
					sCatName, sRoomName);
		} catch (Exception e) {
			// If index was invalid, Null pointer Exception will be thrown
			return 0;
	...
	public DxAvailability getRoomAvailability(String sSiteName,
		...
		try {
			return ((DxSite) this.getResource(sSiteName)).getRoomAvailability(
					sCatName, sRoomName);
		} catch (Exception e) {
			// If index was invalid, Null pointer Exception will be thrown
			return null;
...
//	}
	...
	public DxSetOfRooms getAllDxRooms() {
		...
		while (itSites.hasNext()) {
			dxsCurrentSite = (DxSite) itSites.next();
			DxSetOfCategories dxSofC = dxsCurrentSite.getAllDxCategories();
