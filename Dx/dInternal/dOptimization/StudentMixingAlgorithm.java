/**
 * 
 * Title: StudentMixingAlgorithm $Revision: 1.55 $ $Date: 2007-10-23 18:19:49 $
...
public class StudentMixingAlgorithm implements Algorithm {
	...
	private DModel _dm;
	private Vector<String> _eventsRescList;
	...
	private int getAcceptableVariation() {
		...
		case 0:
			...
			break;
		case 1:
			value = MEDIANACCEPTABLEVARIATION;//intermediaire
			break;
		case 2:
			value = 50;
			break;
		case 3:
			value = PERSONALACCEPTABLEVARIATION;
			break;
		}// end  switch(choice)
		return value;
	}
	 ...
	 */
	public void doWork() {
		Vector<String> eventRescList = buildEventsVector();
		for (int i = 0; i < 2; i++)
	 ...
	 * @param vectorOfEvents
	 */
	private void compileStudents(Vector<String> eventRescList) {
		...
		for (int i = 0; i < _eventsRescList.size(); i++) {
			...
			String actID = DXToolsMethods.getToken(_eventsRescList.get(i)
					.toString(), ".", 0);
			String typeID = DXToolsMethods.getToken(_eventsRescList.get(i)
					.toString(), ".", 1);
			Type type = _dm.getSetOfActivities().getType(actID, typeID);
			Vector<String> studentRegistered = buildStudentsRegistered(actID, typeID);
			for (int j = 0; j < type.getSetOfSections().size(); j++) {
				...
				DSetOfResources associatesEvents = buildOtherAssociatesEvents(
						actID, typeID, section.getID());
				DSetOfResources convGroup = buildConvexGroup(associatesEvents,
						studentRegistered);
	 ...
	 * @return
	 */
	private Vector<String> buildEventsVector() {
		...
		for (int i = 0; i < _dm.getSetOfEvents().size(); i++) {
			...
			if (((DxEvent) rescEvent.getAttach()).isPlaceInAPeriod()) {
				String actID = DXToolsMethods.getToken(rescEvent.getID(), ".",
						0);
				String typeID = DXToolsMethods.getToken(rescEvent.getID(), ".",
						1);
				Type type = _dm.getSetOfActivities().getType(actID, typeID);
				String str = actID + "." + typeID + ".";
				if ((type.getSetOfSections().size() > 1)
						&& (!vect.contains(str)))
					vect.add(str);
			}
	 ...
	 * @return
	 */
	private DSetOfResources buildOtherAssociatesEvents(String activityID,
			String typeID, String sectionID) {
		...
		for (int j = 0; j < ((Section) section.getAttach()).getSetOfUnities()
				.size(); j++) {
			DResource unity = ((Section) section.getAttach()).getSetOfUnities()
					.getResourceAt(j);
			String eventID = activityID + "." + typeID + "." + section.getID()
					+ "." + unity.getID() + ".";
			DxEvent event = (DxEvent) _dm.getSetOfEvents().getResource(eventID)
					.getAttach();
			if (event.isPlaceInAPeriod()) {
				...
				for (int k = 0; k < duration; k++) {
					Period period = _dm.getTTStructure().getCurrentCycle()
							.getPeriodByKey(dayKey, seqKey, perKey + k);
					for (int l = 0; l < period.getEventsInPeriod().size(); l++) {
						String inPeriodEventID = period.getEventsInPeriod()
								.getResourceAt(l).getID();
						if (!eventID.equalsIgnoreCase(inPeriodEventID)) {
							DResource assEvent = assEvents
									.getResource(inPeriodEventID);
							if (assEvent != null) {
								((DValue) assEvent.getAttach())
										.setIntValue(((DValue) assEvent
												.getAttach()).getIntValue() + 1);
							} else {// end if(assEvent!=null)
								DValue value = new DValue();
								value.setIntValue(1);
								assEvents.addResource(new DResource(period
										.getEventsInPeriod().getResourceAt(l)
										.getID(), value), 1);
	 ...
	 * @return
	 */
	private Vector<String> buildStudentsRegistered(String activityID, String typeID) {
		Vector<String> studentR = new Vector<String>(1);
		Activity activity = (Activity) _dm.getSetOfActivities().getResource(
				activityID).getAttach();
		Vector<String> studentList = activity.getStudentRegistered();
	 ...
	 * @return
	 */
	private DSetOfResources buildConvexGroup(DSetOfResources associatesEvents,
			Vector studentsReg) {
		/**
		...
		for (int i = 0; i < studentsReg.size(); i++) {
			...
			for (int j = 0; j < student.getCoursesList().size(); j++) {
				String actID = student.getCoursesList().getResourceAt(j)
						.getID();
				String typeID = actID.substring(
						SetOfActivities._COURSENAMELENGTH,
						SetOfActivities._COURSENAMELENGTH + 1);
				...
				for (int k = 0; k < groupList.size(); k++) {
					...
					Section section = _dm.getSetOfActivities().getSection(
							actID, typeID, sectionID);
					if (section != null) {
						for (int l = 0; l < section.getSetOfUnities().size(); l++) {
							String eventID = actID
									+ "."
									+ typeID
									+ "."
									+ sectionID
									+ "."
									+ section.getSetOfUnities()
											.getResourceAt(l).getID() + ".";
							DResource assEvent = associatesEvents
									.getResource(eventID);
							if (assEvent != null) {
								nbOfPotentialConf += ((DValue) assEvent
			...
			convexGroup.addResource(new DResource(Integer
					.toString(nbOfPotentialConf), new DValue()), 0);
	 ...
	 * @return
	 */
	private DResource getStudent(Vector allConvGroup, DValue currentConvGroup,
			Vector sizeOfGroups) {
		DResource resc = null;
		if (quickSolution(allConvGroup))
			resc = giveBestGroup(resc, allConvGroup, currentConvGroup,
					sizeOfGroups, 1);
		else
			resc = giveBestGroup(resc, allConvGroup, currentConvGroup,
					sizeOfGroups, getAcceptableVariation());
		return resc;
	}
	...
	private DResource giveBestGroup(DResource resc, Vector allConvGroup,
			DValue currentConvGroup, Vector sizeOfGroups,
			int acceptableVariation) {
		...
		for (int i = 0; i < allConvGroup.size(); i++) {
			if ((((DValue) sizeOfGroups.get(i)).getIntValue() - ((DValue) sizeOfGroups
					.get(smallGroup)).getIntValue()) < acceptableVariation)
		...
		if (((DSetOfResources) allConvGroup.get(bGroup)).size() > 0) {
			...
			int bConf = Integer.parseInt(((DSetOfResources) allConvGroup
					.get(bGroup)).getResourceAt(0).getID());

			for (int i = 1; i < includeGroupsList.size(); i++) {
				if (bConf > Integer.parseInt(((DSetOfResources) allConvGroup
						.get((includeGroupsList.get(i)).intValue()))
						.getResourceAt(0).getID())) {
					bConf = Integer.parseInt(((DSetOfResources) allConvGroup
							.get((includeGroupsList.get(i)).intValue()))
							.getResourceAt(0).getID());
			...
			((DValue) sizeOfGroups.get(bGroup))
					.setIntValue(((DValue) sizeOfGroups.get(bGroup))
							.getIntValue() + 1);

			resc1 = ((DSetOfResources) allConvGroup.get(bGroup))
					.getResourceAt(0);
			((DValue) resc.getAttach()).setIntValue(bGroup);
			for (int i = 0; i < allConvGroup.size(); i++)
				((DSetOfResources) allConvGroup.get(i)).removeResource(resc
						.getKey());
			currentConvGroup.setIntValue(bGroup);
