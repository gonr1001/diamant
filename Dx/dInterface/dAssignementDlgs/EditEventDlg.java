...
//                      getResource(activityName));
          unities.add(_dApplic.getCurrentDModel().getSetOfEvents().getResource(activityName));
		...
		if (typID.length() != 0) {
			...
			if (secID.length() != 0) {
				...
				if (unitID.length() != 0) {
					...
					Section sect = _dm.getSetOfActivities().getSection(
							actID, typID, secID);
					for (int i = 0; i < sect.getSetOfUnities().size(); i++) {
						events.add(_dm.getSetOfEvents().getResource(
								actID
  ...
  private String buildDuration(){
    DxEvent event= (DxEvent)((DResource)_unities.get(_currentActivityIndex)).getAttach();
...
/*  private Vector[] buildCapacityList() {
  ...
  private boolean applyChanges(){
    ...
    DxEvent event= (DxEvent)((DResource)_unities.get(_currentActivityIndex)).getAttach();
    //remove event
    _dm.getConditionsToTest().removeEventInTTs(_dm.getTTStructure(),(DResource)_unities.get(_currentActivityIndex),false);
    ...
    event.setKey(2,Long.toString(getResourceKey(_dm.getDxSetOfRooms(),room)));
    ...
    //add event
    _dm.getConditionsToTest().addEventInTTs(_dm.getTTStructure(),(DResource)_unities.get(_currentActivityIndex),false);
    return true;
  }
