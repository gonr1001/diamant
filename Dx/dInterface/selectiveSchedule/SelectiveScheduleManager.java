...
public class SelectiveScheduleManager {
	 ...
	 *  
	 */
	public class PersistanceMismatchManagerImpl implements
			PersistanceMismatchManager {
		/**
	...
	private static Logger logger = Logger
			.getLogger(SelectiveScheduleManager.class);
	...
	private void initializeRelationTesters() {
		...
		_tupleToRelationTester.put(new Tuple(new Class[] { EventAttach.class,
	...
	public boolean validateElement(DResource element) {
		...
		while (itrAllFilterSets.hasNext() && !matchFound) {
			...
			while (itrFilterSet.hasNext()) {
				...
				if (rt == null) {
					logger
							.warn("aucun RelationTester trouvé pour valider l'élément "
									+ element);
	...
	public void writeToPersistenceSink()
		...
		while (itrFilterSet.hasNext()) {
			...
			setElement.setAttribute("name", fs.getFilterSetIdentifier()
					.getFilterSetName());
	...
	public void readFromPersistenceSource()
		...
		if ((_lastXMLPersistanceFileName == null)
				|| (_lastXMLPersistanceFileName
						.equals(fetchXMLPersistanceFileName()) == false)) {
			...
			if (_lastXMLPersistanceFileName == null) {
				logger
						.error("le chargement des ensembles n'a pas fonctionné, car le nom du fichier d'importation automatique de données n'a pas été inscrit");
				JOptionPane
						.showMessageDialog(
								SelectiveScheduleDlg.getInstance(),
		...
		if (fileIn.canRead() == false) {
			logger.error("impossible de lire le fichier XML "
					+ fileIn.getName());
			throw new SelectiveScheduleFileIOException(
					SelectiveScheduleManager.class
		...
		try {
			...
			JOptionPane.showMessageDialog(DApplication.getInstance()
					.getJFrame(), "Erreur DOM lors de la lecture du fichier "
					+ fileIn.getName(), "Erreur de lecture de fichier",
					JOptionPane.ERROR_MESSAGE);
		} catch (IOException e1) {
			logger.error(e1);
			JOptionPane.showMessageDialog(DApplication.getInstance()
					.getJFrame(), "Erreur I/O lors de la lecture du fichier "
					+ fileIn.getName(), "Erreur de lecture de fichier",
					JOptionPane.ERROR_MESSAGE);
		}
		...
		while (itrSetsElement.hasNext()) {
			...
			try {
				fs.getFilterSetIdentifier().setFilterSetName(
						setElement.getAttributeValue("name"));
			...
			while (itrSetElement.hasNext()) {
				...
				activity = DApplication.getInstance().getCurrentDModel()
						.getSetOfActivities().getResource(activityID);
	...
	private String fetchXMLPersistanceFileName() {
		return DApplication.getInstance().getCurrentDxDoc()
				.getAutoImportDIMFilePath()
				+ "fgs.xml";
	}
} // end SelectiveScheduleManager
