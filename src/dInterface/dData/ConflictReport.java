...
//		}
	...
	public void showReport() {
		...
		if (_rightVec.size() == 0)
			...
			_jTextArea.setText((_parentDlg.getStandardReportData())
					.getConflictsReport(firstField, othersFields));

			buildReport(_rightVec.toArray(), fieldsLengths(_rightVec,
					_allOptionsVec), (_parentDlg.getStandardReportData())
					.getConflictsReport(firstField, othersFields));
	 ...
	 * separated by a ";" separator token number 0= day number, 1= day name, 2=
	...
	private Vector<DValue> buildAllOptionsVector() {
		...
		v.add(new DValue(0, new FieldRecord(Integer
				.parseInt(DConst.R_DAY_NUMBER_L), DConst.R_DAY_NUMBER)));
		v.add(new DValue(1, new FieldRecord(Integer
				.parseInt(DConst.R_DAY_NAME_L), DConst.R_DAY_NAME)));
		v.add(new DValue(2, new FieldRecord(Integer
				.parseInt(DConst.R_SEQUENCE_ID_L), DConst.R_SEQUENCE_ID)));
		// v.add(new DXValue(3,
		// new FieldRecord(Integer.parseInt(DConst.R_UNITY_NAME_L),
		// DConst.R_UNITY_NAME)));
		v.add(new DValue(4, new FieldRecord(Integer
				.parseInt(DConst.R_PERIOD_BEGIN_H_L), DConst.R_PERIOD_BEGIN)));
		v.add(new DValue(5, new FieldRecord(Integer
				.parseInt(DConst.R_EVENT1_ID_L), DConst.R_EVENT1_ID)));
		v.add(new DValue(6, new FieldRecord(Integer
				.parseInt(DConst.R_EVENT2_ID_L), DConst.R_EVENT2_ID)));
		v.add(new DValue(7, new FieldRecord(Integer
				.parseInt(DConst.R_NUMBER_OF_CONFLICTS_L),
				DConst.R_NUMBER_OF_CONFLICTS)));
		v.add(new DValue(8, new FieldRecord(Integer
				.parseInt(DConst.R_TYPE_OF_CONFLICT_L),
				DConst.R_TYPE_OF_CONFLICT)));
		v.add(new DValue(9, new FieldRecord(Integer
				.parseInt(DConst.R_ELEMENTS_IN_CONFLICT_L),
				DConst.R_ELEMENTS_IN_CONFLICT)));

		return v;
	}

	/*
	 * private Vector getOptions(Vector opt) { Vector v = new Vector(); for (int
	 * i=0; i< opt.size(); i++) //v.add(((FieldRecord) opt.get(i))._str);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getActionCommand().equals(DConst.BUT_OPTIONS))
			new ReportOptionsDlg(_dApplic, this, _options, _elements);
		// _elements = _options.size() - _rightVec.size();
		// _options = buildExternalOptions(_options, _rightVec);
		showReport();
		// if "Close" button
		if (e.getActionCommand().equals(DConst.BUT_CLOSE))
			// _dApplic.getPreferences().setSelectedOptionsInConflictReport(
			// _rightVec);
		...
		if (e.getActionCommand().equals(DConst.BUT_SAVE_AS)) {
			...
			String data = "***** " + DConst.REPORT + " " + DConst.TO_LEFT
					+ DConst.REPORT_DLG_TAB2 + DConst.TO_RIGHT + "          ";
...
//	}
	...
	public void doSavePref(Vector<String> rigth) {
		new ReportPref().saveSelectedOptionsInConflictReport(rigth);
		_rightVec = rigth;
	}
}
